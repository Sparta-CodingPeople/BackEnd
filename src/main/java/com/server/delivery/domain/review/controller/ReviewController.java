...
public class ReviewController {
    ...
    public CustomResponse<ReviewCreateResponseDto> createReview(@AuthenticationPrincipal CustomUserDetail userDetail, @RequestPart("review") ReviewCreateRequestDto request, @RequestPart(value = "reviewImages", required = false) List<MultipartFile> images) {
    ...
    public CustomResponse<ReviewUpdateResponseDto> updateReview(@PathVariable UUID reviewId, @RequestPart("review") ReviewUpdateRequestDto request, @RequestPart(value = "reviewImages", required = false) List<MultipartFile> images) {
    ...
    public CustomResponse<PagedModel<StoreReviewSearchResponseDto>> searchStoreReviews(
            ...
            @PageableDefault(sort = {"createdAt", "modifiedAt"}, direction = Sort.Direction.DESC, page = 0, size = 10) Pageable pageable
    ) {
        PagedModel<StoreReviewSearchResponseDto> reviewPage = reviewService.searchStoreReviews(storeId, keyword, pageable);
    ...
    public CustomResponse<PagedModel<UserReviewSearchResponseDto>> searchUserReviews(
        ...
        PagedModel<UserReviewSearchResponseDto> reviewPage = reviewService.searchUserReviews(userDetail.getUserId(), keyword, pageable);
